...
pub fn get_tree(path: &str) -> Result<Vec<UfedTreeEntry>, ContainerError> {
    ...
    match format {
        UfedFormat::UfedZip => {
            ...
            let index = crate::archive_ops::get_zip_index(path)?;
            ...
            let entries = crate::archive_ops::list_zip_entries(path)?;
            Ok(entries
                .into_iter()
                .map(|e| UfedTreeEntry {
                    path: e.path.clone(),
...
pub fn get_entry_count(path: &str) -> Result<usize, ContainerError> {
    ...
    match format {
        UfedFormat::UfedZip => crate::archive_ops::get_zip_entry_count(path),
        _ => {
