...
// Re-export everything from sub-modules for crate-internal use
...
use crate::types::*;
use ffx_common::segments::{build_ad1_segment_path, discover_ad1_segments};
...
pub(crate) fn segment_header_info(header: &SegmentHeader) -> SegmentHeaderInfo {
    SegmentHeaderInfo {
...
pub(crate) fn logical_header_info(header: &LogicalHeader) -> LogicalHeaderInfo {
    LogicalHeaderInfo {
        signature: bytes_to_string(&header.signature, false),
...
pub(crate) fn apply_metadata(path: &Path, metadata: &[Metadata]) -> Result<(), ContainerError> {
