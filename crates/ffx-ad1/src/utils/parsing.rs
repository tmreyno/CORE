...
use crate::types::*;
use ffx_common::binary::{read_string_at, read_u32_at, read_u64_at};
...
pub(crate) fn read_segment_header(file: &mut File) -> Result<SegmentHeader, ContainerError> {
...
pub(crate) fn read_logical_header(file: &mut File) -> Result<LogicalHeader, ContainerError> {
...
pub fn validate_ad1(path: &str, check_segments: bool) -> Result<(), ContainerError> {
    ...
    if check_segments {
        for index in 1..=segment_count {
            let segment_path = ffx_common::segments::build_ad1_segment_path(path, index);
...
pub(crate) fn find_hash(metadata: &[Metadata], key: u32) -> Option<String> {
...
#[allow(dead_code)]
pub(crate) fn find_sha256_hash(metadata: &[Metadata]) -> Option<String> {
...
pub(crate) fn find_timestamp(metadata: &[Metadata], key: u32) -> Option<String> {
    metadata
        .iter()
