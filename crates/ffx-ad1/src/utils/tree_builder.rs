...
pub(crate) fn find_item_by_path<'a>(items: &'a [Item], target_path: &str) -> Option<FoundItem<'a>> {
...
/// * `parent_path` - Path to the parent of this item
pub(crate) fn build_tree_entry_lazy(item: &Item, parent_path: &str) -> TreeEntry {
    let path = join_path(parent_path, &item.name);
    let is_dir = item.item_type == AD1_FOLDER_SIGNATURE;
    let size = if is_dir { 0 } else { item.decompressed_size };
...
pub(crate) fn collect_tree(items: &[Item], parent_path: &str, out: &mut Vec<TreeEntry>) {
    for item in items {
...
#[allow(dead_code)]
pub(crate) fn collect_tree_simple(items: &[Item], parent_path: &str, out: &mut Vec<TreeEntry>) {
    for item in items {
...
pub(crate) fn collect_children_at_path(
    items: &[Item],
    target_path: &str,
    current_path: &str,
...
pub(crate) fn count_files(items: &[Item]) -> u64 {
    items
        .iter()
        .map(|item| {
