    ...
    F: FnMut(u64, u64),
{
    ...
    for item in &root_items {
        session.extract_item_with_progress(
            item,
            output_path,
            &mut current,
            total,
...
pub fn export_tree_json(path: &str) -> Result<String, ContainerError> {
    ...
    serde_json::to_string_pretty(&entries).map_err(|e| {
        ContainerError::SerializationError(format!("Failed to serialize tree to JSON: {e}"))
    })
}
...
pub fn export_tree_csv(path: &str) -> Result<String, ContainerError> {
    let entries = super::get_tree(path)?;
    let mut csv =
        String::from("path,is_dir,size,item_type,md5_hash,sha1_hash,created,accessed,modified\n");
...
pub fn export_metadata_json(path: &str) -> Result<String, ContainerError> {
    ...
    serde_json::to_string_pretty(&metadata).map_err(|e| {
        ContainerError::SerializationError(format!("Failed to serialize metadata to JSON: {e}"))
    })
}
...
pub fn export_metadata_csv(path: &str) -> Result<String, ContainerError> {
    ...
    csv.push_str(&format!(
        ...
        ffx_common::format_size(total_size)
    ));
    csv.push('\n');
