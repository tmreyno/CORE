...
impl Ad1Vfs {
    ...
    fn get_session(&self) -> Result<(), VfsError> {
        ...
        if needs_open {
            let new_session =
                Session::open(&self.path).map_err(|e| VfsError::IoError(e.to_string()))?;
    ...
    fn load_children(&self, path: &str) -> Result<Vec<String>, VfsError> {
        ...
        let session = session_guard.as_ref().ok_or(VfsError::NotMounted)?;
    ...
    fn find_items_at_path<'a>(
        &self,
        session: &'a Session,
        path: &str,
    ) -> Result<&'a [Item], VfsError> {
    ...
    fn item_to_attr(&self, item: &Item, inode: u64) -> FileAttr {
        ...
        let created = find_timestamp(&item.metadata, CREATED).and_then(|s| parse_timestamp(&s));
        let accessed = find_timestamp(&item.metadata, ACCESS).and_then(|s| parse_timestamp(&s));
        let modified = find_timestamp(&item.metadata, MODIFIED).and_then(|s| parse_timestamp(&s));
...
impl VirtualFileSystem for Ad1Vfs {
    fn getattr(&self, path: &str) -> Result<FileAttr, VfsError> {
        ...
        if let Some(parent) = ffx_common::vfs::parent_path(&normalized) {
    ...
    fn readdir(&self, path: &str) -> Result<Vec<DirEntry>, VfsError> {
        ...
        for child_path in &child_paths {
            if let Some(entry) = cache.get(child_path) {
                let name = ffx_common::vfs::filename(child_path).to_string();
