...
static MAGIC_SIGNATURES: &[MagicSignature] = &[
    ...
    MagicSignature::new(
        ...
        21,
        "application/x-vmdk",
        "VMware Virtual Disk (Descriptor)",
        &["vmdk"],
        FileCategory::Forensic,
    ),
    MagicSignature::new(
...
pub fn detect_file_type(header: &[u8]) -> Option<FileType> {
    ...
    if header.len() >= 2 && header[0] == 0xFF && (header[1] & 0xE0) == 0xE0 {
        return Some(
            FileType::new("audio/mpeg", "MP3 Audio", &["mp3"], FileCategory::Audio)
...
fn detect_riff_type(header: &[u8]) -> Option<FileType> {
    ...
    match &header[8..12] {
        b"WEBP" => Some(FileType::new(
            "image/webp",
            "WebP Image",
            &["webp"],
            FileCategory::Image,
        )),
        b"WAVE" => Some(FileType::new(
            "audio/wav",
            "WAV Audio",
            &["wav"],
            FileCategory::Audio,
        )),
        b"AVI " => Some(FileType::new(
            "video/avi",
            "AVI Video",
            &["avi"],
            FileCategory::Video,
...
fn detect_ftyp_type(header: &[u8]) -> Option<FileType> {
    ...
    match brand {
        b"heic" | b"heix" | b"hevc" | b"mif1" => Some(FileType::new(
            "image/heic",
            "HEIC Image",
            &["heic", "heif"],
            FileCategory::Image,
        )),
        b"isom" | b"iso2" | b"mp41" | b"mp42" => Some(FileType::new(
            "video/mp4",
            "MP4 Video",
            &["mp4", "m4v"],
            FileCategory::Video,
        )),
        b"qt  " => Some(FileType::new(
            "video/quicktime",
            "QuickTime Video",
            &["mov"],
            FileCategory::Video,
