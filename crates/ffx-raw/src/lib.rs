...
use ffx_common::{hash::StreamingHasher, segments::discover_numbered_segments, BUFFER_SIZE};
...
pub fn export_metadata_json(path: &str) -> Result<String, ContainerError> {
    ...
    let segments: Vec<SegmentDetail> = info
        ...
        .map(|(i, (name, &size))| SegmentDetail {
            ...
            name: name.clone(),
            size,
            size_formatted: ffx_common::format_size(size),
        })
        .collect();
