...
use ffx_common::segments::discover_e01_segments;
...
pub fn info(path: &str) -> Result<EwfInfo, ContainerError> {
    ...
    let file_timestamp: Option<String> = Path::new(path)
        .metadata()
        .ok()
    ...
    let stored_hashes: Vec<StoredImageHash> = handle
        .stored_hashes
        .iter()
        .map(|h| StoredImageHash {
...
pub fn is_e01(path: &str) -> Result<bool, ContainerError> {
    ...
    debug!(
        "is_e01: {} -> sig={:02x?} ewf1={} ewf2={} lvf1={} lvf2={}",
...
pub fn get_stats(path: &str) -> Result<super::types::EwfStats, ContainerError> {
    ...
    let has_md5 = handle
        .stored_hashes
        .iter()
        .any(|h| h.algorithm.to_lowercase() == "md5");
    let has_sha1 = handle
        .stored_hashes
        .iter()
        .any(|h| h.algorithm.to_lowercase() == "sha1");
...
pub fn verify_chunks_detailed(
    path: &str,
    _algorithm: &str,
) -> Result<Vec<super::types::ChunkVerifyResult>, ContainerError> {
...
where
    F: FnMut(u64, u64),
{
    ffx_common::hash_segment_with_progress(segment_path, algorithm, progress_callback)
}
...
fn verify_with_progress_optimized<F>(
    path: &str,
    algorithm: &str,
    mut progress_callback: F,
) -> Result<String, ContainerError>
    ...
    F: FnMut(u64, u64),
{
    ...
    let is_known_algo = use_sha1
        ...
        || use_xxh64
        || use_crc32;
    let mut md5_hasher: Option<Md5> = if !is_known_algo {
        Some(Md5::new())
    } else {
        None
    };
    let mut sha1_hasher = if use_sha1 { Some(Sha1::new()) } else { None };
    let mut sha256_hasher = if use_sha256 {
        Some(Sha256::new())
    } else {
        None
    };
    let mut sha512_hasher = if use_sha512 {
        Some(Sha512::new())
    } else {
        None
    };
    let mut blake3_hasher = if use_blake3 {
        Some(Blake3Hasher::new())
    } else {
        None
    };
    let mut blake2_hasher = if use_blake2 {
        Some(Blake2b512::new())
    } else {
        None
    };
    ...
    let mut crc32_hasher = if use_crc32 {
...
#[allow(dead_code)]
fn verify_with_progress_parallel_chunks<F>(
    path: &str,
    algorithm: &str,
    mut progress_callback: F,
) -> Result<String, ContainerError>
    ...
    F: FnMut(u64, u64),
{
    ...
    let (tx, rx) =
        mpsc::sync_channel::<Result<(usize, Vec<Vec<u8>>), ContainerError>>(channel_depth);
    ...
    let decompression_handle = thread::spawn(move || {
        ...
        for batch_start in (0..chunk_count).step_by(batch_size) {
            ...
            let batch_data: Vec<Vec<u8>> =
                indexed_chunks.into_iter().map(|(_, data)| data).collect();
