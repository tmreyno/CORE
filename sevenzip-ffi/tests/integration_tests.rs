...
use seven_zip::{CompressOptions, CompressionLevel, SevenZip};
use std::fs;
...
fn test_compression_levels() {
    ...
    for level in levels {
        ...
        assert!(
            result.is_ok(),
            "Compression with {:?} should succeed",
            level
        );
        assert!(
            archive_path.exists(),
            "Archive should exist for {:?}",
            level
        );
    }
}
...
fn test_encryption_direct() {
    ...
    assert!(
        ...
        "Ciphertext should be >= plaintext due to padding"
    );
...
fn test_encryption_with_wrong_password() {
    use seven_zip::encryption::{DecryptionContext, EncryptionContext};
    ...
    assert!(
        dec_ctx_result.is_ok(),
        "DecryptionContext should create even with wrong password"
    );
}
...
fn test_compress_options() {
    ...
    assert!(
        result.is_ok(),
        "Archive creation with custom options should succeed"
    );
...
fn test_progress_callback() {
    ...
    // Create archive
    sz.create_archive(&archive_path, &[&test_file], CompressionLevel::Normal, None)
        .unwrap();
    ...
    let result = sz.extract(&archive_path, &extract_dir);
    assert!(
        result.is_ok(),
    ...
    assert!(
        result.is_ok(),
        "Extraction with callback should succeed: {:?}",
        result.err()
    );
...
fn test_streaming_compression_with_progress() {
    ...
    let result = sz.create_archive_streaming(
        ...
        Some(&opts),
        Some(Box::new(
            move |processed, total, file_bytes, file_total, filename| {
                *progress_count_clone.lock().unwrap() += 1;
                println!(
                    "Processing {}: {}/{} bytes (total: {}/{})",
    ...
    assert!(
        result.is_ok(),
        "Streaming compression should succeed: {:?}",
        result.err()
    );
    assert!(archive_path.exists(), "Archive should exist");
...
fn test_split_archive_creation() {
    ...
    match result {
        Ok(()) => {
            ...
            assert!(
                archive_path.exists() || temp.path().join("split.7z.001").exists(),
...
fn test_streaming_with_password() {
    ...
    let result = sz.extract_with_password(&archive_path, &extract_dir, Some(password), None);
...
fn test_incompressible_data_detection() {
    ...
    assert!(
        duration_auto < duration_normal / 2,
        "Auto-detect not faster: {:?} vs {:?}",
        duration_auto,
        duration_normal
    );
}
...
fn test_smart_threading() {
    ...
    assert!(
        small_opts.auto_detect_incompressible,
        "Should enable auto-detect by default"
    );
}
