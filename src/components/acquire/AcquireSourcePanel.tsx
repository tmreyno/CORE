// =============================================================================
// CORE-FFX - Forensic File Explorer
// Copyright (c) 2024-2026 CORE-FFX Project Contributors
// Licensed under MIT License - see LICENSE file for details
// =============================================================================

/**
 * AcquireSourcePanel — Left sidebar for CORE Acquisition edition.
 *
 * Displays two collapsible sections:
 *   1. Drives & Volumes  — Mounted drives with expandable directory trees
 *   2. Image Files       — Manually added forensic image files
 *
 * Drive entries expand to show their directory tree (lazy-loaded via
 * the `list_directory` Tauri command). Files and folders within the
 * tree are selectable and support right-click context menu for
 * acquire actions.
 */

import {
  Component,
  Show,
  For,
  createSignal,
  createMemo,
  onMount,
} from "solid-js";
import { invoke } from "@tauri-apps/api/core";
import {
  HiOutlineCircleStack,
  HiOutlineDocument,
  HiOutlineArrowPath,
  HiOutlineComputerDesktop,
  HiOutlineServer,
  HiOutlinePlusCircle,
  HiOutlineXMark,
  HiOutlineChevronRight,
  HiOutlineChevronDown,
  HiOutlineFolder,
  HiOutlineFolderOpen,
  HiOutlineCheckCircle,
  HiOutlineQueueList,
  HiOutlineTrash,
} from "../icons";
import { createContextMenu, ContextMenu, type ContextMenuItem } from "../ContextMenu";
import type { DriveInfo } from "../../api/drives";
import { listDrives, formatDriveSize } from "../../api/drives";
import { formatBytes } from "../../utils";

// =============================================================================
// Types
// =============================================================================

/** Matches the Rust DirEntry struct (serde camelCase) */
interface FsDirEntry {
  name: string;
  path: string;
  isDir: boolean;
  size: number;
  modified: number | null;
}

/** An item queued for acquisition */
export interface AcquireQueueItem {
  path: string;
  name: string;
  isDir: boolean;
  size: number;
}

export interface AcquireSourcePanelProps {
  /** Called when user wants to acquire source(s) as E01 */
  onAcquirePhysical: (sourcePaths: string[]) => void;
  /** Called when user wants to acquire source(s) as L01 */
  onAcquireLogical: (sourcePaths: string[]) => void;
  /** Manually-added image file paths */
  imageFiles: () => string[];
  /** Add image file(s) */
  onAddImageFiles: () => void;
  /** Remove an image file by index */
  onRemoveImageFile: (index: number) => void;
  /** Called when the acquisition queue changes */
  onQueueChange?: (items: AcquireQueueItem[]) => void;
}

// =============================================================================
// Helpers
// =============================================================================

const basename = (path: string): string => {
  const parts = path.replace(/\\/g, "/").split("/");
  return parts[parts.length - 1] || path;
};

const driveIcon = (drive: DriveInfo) => {
  if (drive.isRemovable) return HiOutlineCircleStack;
  if (drive.isSystemDisk) return HiOutlineComputerDesktop;
  return HiOutlineServer;
};

// =============================================================================
// Component
// =============================================================================

const AcquireSourcePanel: Component<AcquireSourcePanelProps> = (props) => {
  const [drives, setDrives] = createSignal<DriveInfo[]>([]);
  const [drivesLoading, setDrivesLoading] = createSignal(false);
  const [drivesExpanded, setDrivesExpanded] = createSignal(true);
  const [imagesExpanded, setImagesExpanded] = createSignal(true);

  // Tree expansion state (keyed by directory path)
  const [expandedPaths, setExpandedPaths] = createSignal<Set<string>>(new Set());
  const [dirChildren, setDirChildren] = createSignal<Map<string, FsDirEntry[]>>(new Map());
  const [loadingPaths, setLoadingPaths] = createSignal<Set<string>>(new Set());

  // Acquisition queue — items selected for acquisition with metadata
  const [queueItems, setQueueItems] = createSignal<AcquireQueueItem[]>([]);
  const [queueExpanded, setQueueExpanded] = createSignal(true);

  const contextMenu = createContextMenu();

  // Derived: quick lookup set for checking if a path is queued
  const queuedPaths = createMemo(() => new Set(queueItems().map(i => i.path)));

  // Separate system drives and external/removable drives
  const externalDrives = createMemo(() =>
    drives().filter(d => !d.isSystemDisk)
  );
  const systemDrives = createMemo(() =>
    drives().filter(d => d.isSystemDisk)
  );

  const queueCount = createMemo(() => queueItems().length);
  const queueTotalSize = createMemo(() =>
    queueItems().reduce((sum, i) => sum + (i.isDir ? 0 : i.size), 0)
  );

  // ── Drive loading ─────────────────────────────────────────────────────────

  const loadDrives = async () => {
    setDrivesLoading(true);
    try {
      const list = await listDrives();
      setDrives(list);
    } catch {
      // Silently handle
    } finally {
      setDrivesLoading(false);
    }
  };

  onMount(() => {
    loadDrives();
  });

  // ── Tree expansion ────────────────────────────────────────────────────────

  const toggleExpand = async (dirPath: string) => {
    const expanded = new Set(expandedPaths());

    if (expanded.has(dirPath)) {
      expanded.delete(dirPath);
      setExpandedPaths(expanded);
      return;
    }

    // Load children if not cached
    if (!dirChildren().has(dirPath)) {
      const loading = new Set(loadingPaths());
      loading.add(dirPath);
      setLoadingPaths(loading);
      try {
        const entries = await invoke<FsDirEntry[]>("list_directory", { path: dirPath });
        const children = new Map(dirChildren());
        children.set(dirPath, entries);
        setDirChildren(children);
      } catch {
        // Permission denied or inaccessible — silently handle
      } finally {
        const l = new Set(loadingPaths());
        l.delete(dirPath);
        setLoadingPaths(l);
      }
    }

    expanded.add(dirPath);
    setExpandedPaths(expanded);
  };

  // ── Acquisition Queue ──────────────────────────────────────────────────────

  const addToQueue = (path: string, name: string, isDir: boolean, size: number) => {
    if (queuedPaths().has(path)) return; // already queued
    const item: AcquireQueueItem = { path, name, isDir, size };
    const next = [...queueItems(), item];
    setQueueItems(next);
    props.onQueueChange?.(next);
  };

  const removeFromQueue = (path: string) => {
    const next = queueItems().filter(i => i.path !== path);
    setQueueItems(next);
    props.onQueueChange?.(next);
  };

  const toggleQueue = (path: string, name: string, isDir: boolean, size: number) => {
    if (queuedPaths().has(path)) {
      removeFromQueue(path);
    } else {
      addToQueue(path, name, isDir, size);
    }
  };

  const clearQueue = () => {
    setQueueItems([]);
    props.onQueueChange?.([]);
  };

  // ── Context menu builder ──────────────────────────────────────────────────

  const buildSourceMenuItems = (path: string, isDir: boolean): ContextMenuItem[] => {
    const items: ContextMenuItem[] = [
      {
        id: "acquire-e01",
        label: "Acquire as E01 (Disk Image)",
        icon: "💿",
        onSelect: () => props.onAcquirePhysical([path]),
      },
      {
        id: "acquire-l01",
        label: "Acquire as L01 (Logical)",
        icon: "📦",
        onSelect: () => props.onAcquireLogical([path]),
      },
      { id: "sep1", label: "", separator: true },
    ];

    // Add queue toggle
    const isQueued = queuedPaths().has(path);
    items.push({
      id: "toggle-queue",
      label: isQueued ? "Remove from Queue" : "Add to Queue",
      icon: isQueued ? "➖" : "➕",
      onSelect: () => toggleQueue(path, basename(path), isDir, 0),
    });

    if (isDir) {
      items.push({
        id: "expand",
        label: expandedPaths().has(path) ? "Collapse" : "Expand",
        icon: "📂",
        onSelect: () => toggleExpand(path),
      });
    }

    items.push(
      { id: "sep2", label: "", separator: true },
      {
        id: "copy-path",
        label: "Copy Path",
        icon: "📋",
        onSelect: () => navigator.clipboard.writeText(path),
      },
    );

    return items;
  };

  const handleDriveContextMenu = (drive: DriveInfo, e: MouseEvent) => {
    e.preventDefault();
    contextMenu.open(e, buildSourceMenuItems(drive.mountPoint, true));
  };

  const handleImageContextMenu = (path: string, e: MouseEvent) => {
    e.preventDefault();
    contextMenu.open(e, buildSourceMenuItems(path, false));
  };

  const handleTreeContextMenu = (entry: FsDirEntry, e: MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    contextMenu.open(e, buildSourceMenuItems(entry.path, entry.isDir));
  };

  // ── Acquire queued items ──────────────────────────────────────────────────

  const handleAcquireQueued = () => {
    const items = queueItems();
    if (items.length > 0) {
      props.onAcquireLogical(items.map(i => i.path));
    }
  };

  // ── Directory tree node (recursive) ───────────────────────────────────────

  const DirTreeNode = (nodeProps: { entry: FsDirEntry; depth: number }) => {
    const isExpanded = () => expandedPaths().has(nodeProps.entry.path);
    const isLoading = () => loadingPaths().has(nodeProps.entry.path);
    const isQueued = () => queuedPaths().has(nodeProps.entry.path);
    const children = () => dirChildren().get(nodeProps.entry.path) || [];

    return (
      <>
        <div
          class="acquire-tree-row"
          classList={{
            "acquire-tree-selected": isQueued(),
          }}
          style={{ "padding-left": `${nodeProps.depth * 14 + 6}px` }}
          onClick={(e) => {
            e.stopPropagation();
            if (nodeProps.entry.isDir) {
              toggleExpand(nodeProps.entry.path);
            } else {
              toggleQueue(nodeProps.entry.path, nodeProps.entry.name, false, nodeProps.entry.size);
            }
          }}
          onContextMenu={(e) => handleTreeContextMenu(nodeProps.entry, e)}
          title={nodeProps.entry.path}
        >
          {/* Expand chevron for directories */}
          <Show
            when={nodeProps.entry.isDir}
            fallback={<span class="w-4 shrink-0" />}
          >
            <span class="w-4 h-4 flex items-center justify-center shrink-0 text-txt-muted">
              <Show
                when={!isLoading()}
                fallback={
                  <svg class="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" />
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
                }
              >
                <Show
                  when={isExpanded()}
                  fallback={<HiOutlineChevronRight class="w-3 h-3" />}
                >
                  <HiOutlineChevronDown class="w-3 h-3" />
                </Show>
              </Show>
            </span>
          </Show>

          {/* Queue indicator */}
          <Show when={isQueued()}>
            <HiOutlineCheckCircle class="w-3.5 h-3.5 text-accent shrink-0" />
          </Show>

          {/* File/folder icon */}
          <Show
            when={nodeProps.entry.isDir}
            fallback={<HiOutlineDocument class="w-3.5 h-3.5 text-txt-secondary shrink-0" />}
          >
            <Show
              when={isExpanded()}
              fallback={<HiOutlineFolder class="w-3.5 h-3.5 text-amber-400 shrink-0" />}
            >
              <HiOutlineFolderOpen class="w-3.5 h-3.5 text-amber-400 shrink-0" />
            </Show>
          </Show>

          {/* Name */}
          <span class="flex-1 truncate text-xs text-txt">{nodeProps.entry.name}</span>

          {/* File size */}
          <Show when={!nodeProps.entry.isDir && nodeProps.entry.size > 0}>
            <span class="text-2xs text-txt-muted tabular-nums shrink-0 mr-1">
              {formatBytes(nodeProps.entry.size)}
            </span>
          </Show>
        </div>

        {/* Recursive children */}
        <Show when={nodeProps.entry.isDir && isExpanded()}>
          <Show
            when={children().length > 0}
            fallback={
              <Show when={!isLoading()}>
                <div
                  class="text-2xs text-txt-muted italic"
                  style={{ "padding-left": `${(nodeProps.depth + 1) * 14 + 24}px` }}
                >
                  Empty
                </div>
              </Show>
            }
          >
            <For each={children()}>
              {(child) => <DirTreeNode entry={child} depth={nodeProps.depth + 1} />}
            </For>
          </Show>
        </Show>
      </>
    );
  };

  // ── Render ────────────────────────────────────────────────────────────────

  return (
    <div class="acquire-source-panel">
      {/* Panel header */}
      <div class="acquire-source-header">
        <span class="text-xs font-medium text-txt uppercase tracking-wider">Sources</span>
        <div class="flex items-center gap-1">
          <button
            class="icon-btn-sm"
            onClick={props.onAddImageFiles}
            title="Add image files"
          >
            <HiOutlinePlusCircle class="w-4 h-4" />
          </button>
          <button
            class="icon-btn-sm"
            onClick={loadDrives}
            title="Refresh drives"
            disabled={drivesLoading()}
          >
            <HiOutlineArrowPath class="w-4 h-4" classList={{ "animate-spin": drivesLoading() }} />
          </button>
        </div>
      </div>

      {/* Queue summary bar — shown when items are queued */}
      <Show when={queueCount() > 0}>
        <div class="acquire-selection-bar">
          <span class="text-2xs text-accent font-medium">
            {queueCount()} queued
            <Show when={queueTotalSize() > 0}>
              {" · "}{formatBytes(queueTotalSize())}
            </Show>
          </span>
          <div class="flex items-center gap-1">
            <button
              class="btn-sm text-2xs px-2 py-0.5"
              onClick={handleAcquireQueued}
              title="Acquire queued items"
            >
              Acquire
            </button>
            <button
              class="icon-btn-sm"
              onClick={clearQueue}
              title="Clear queue"
            >
              <HiOutlineXMark class="w-3 h-3" />
            </button>
          </div>
        </div>
      </Show>

      {/* Scrollable content */}
      <div class="acquire-source-body">
        {/* ── Drives Section ─────────────────────────────────── */}
        <div class="acquire-source-section">
          <button
            class="acquire-source-section-header"
            onClick={() => setDrivesExpanded(v => !v)}
          >
            <span class="acquire-source-chevron" classList={{ "rotate-90": drivesExpanded() }}>▶</span>
            <HiOutlineCircleStack class="w-3.5 h-3.5 text-txt-muted" />
            <span class="flex-1 text-left">Drives & Volumes</span>
            <span class="text-2xs text-txt-muted">{drives().length}</span>
          </button>

          <Show when={drivesExpanded()}>
            <Show
              when={!drivesLoading()}
              fallback={
                <div class="px-3 py-3 text-2xs text-txt-muted text-center">
                  Scanning drives…
                </div>
              }
            >
              <Show
                when={drives().length > 0}
                fallback={
                  <div class="px-3 py-3 text-2xs text-txt-muted text-center">
                    No drives detected
                  </div>
                }
              >
                <div class="acquire-source-list">
                  {/* External / removable drives first */}
                  <For each={externalDrives()}>
                    {(drive) => {
                      const Icon = driveIcon(drive);
                      const driveExpanded = () => expandedPaths().has(drive.mountPoint);
                      const driveLoading = () => loadingPaths().has(drive.mountPoint);
                      const driveChildren = () => dirChildren().get(drive.mountPoint) || [];

                      return (
                        <>
                          <button
                            class="acquire-source-item"
                            onContextMenu={(e) => handleDriveContextMenu(drive, e)}
                            onClick={() => toggleExpand(drive.mountPoint)}
                            title={`${drive.mountPoint} — ${drive.fileSystem} — ${formatDriveSize(drive.totalBytes)}\nClick to browse · Right-click for options`}
                          >
                            {/* Expand chevron */}
                            <span class="w-4 h-4 flex items-center justify-center shrink-0 text-txt-muted">
                              <Show
                                when={!driveLoading()}
                                fallback={
                                  <svg class="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" />
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                                  </svg>
                                }
                              >
                                <Show
                                  when={driveExpanded()}
                                  fallback={<HiOutlineChevronRight class="w-3 h-3" />}
                                >
                                  <HiOutlineChevronDown class="w-3 h-3" />
                                </Show>
                              </Show>
                            </span>

                            <Icon class="w-4 h-4 text-blue-400 shrink-0" />
                            <div class="flex-1 min-w-0">
                              <div class="text-xs text-txt truncate">
                                {drive.name || basename(drive.mountPoint)}
                              </div>
                              <div class="text-2xs text-txt-muted truncate">
                                {drive.mountPoint} · {drive.fileSystem.toUpperCase()} · {formatDriveSize(drive.totalBytes)}
                                {drive.isRemovable ? " · USB" : ""}
                              </div>
                            </div>
                            <Show when={drive.isReadOnly}>
                              <span class="text-2xs text-warning" title="Read-only">RO</span>
                            </Show>
                          </button>

                          {/* Drive directory tree */}
                          <Show when={driveExpanded()}>
                            <Show
                              when={driveChildren().length > 0}
                              fallback={
                                <Show when={!driveLoading()}>
                                  <div class="text-2xs text-txt-muted italic px-6 py-1">
                                    Empty or inaccessible
                                  </div>
                                </Show>
                              }
                            >
                              <For each={driveChildren()}>
                                {(entry) => <DirTreeNode entry={entry} depth={1} />}
                              </For>
                            </Show>
                          </Show>
                        </>
                      );
                    }}
                  </For>

                  {/* System drives (dimmed) */}
                  <For each={systemDrives()}>
                    {(drive) => {
                      const Icon = driveIcon(drive);
                      const driveExpanded = () => expandedPaths().has(drive.mountPoint);
                      const driveLoading = () => loadingPaths().has(drive.mountPoint);
                      const driveChildren = () => dirChildren().get(drive.mountPoint) || [];

                      return (
                        <>
                          <button
                            class="acquire-source-item opacity-60"
                            onContextMenu={(e) => handleDriveContextMenu(drive, e)}
                            onClick={() => toggleExpand(drive.mountPoint)}
                            title={`${drive.mountPoint} (System) — ${drive.fileSystem}\nClick to browse · Right-click for options`}
                          >
                            <span class="w-4 h-4 flex items-center justify-center shrink-0 text-txt-muted">
                              <Show
                                when={!driveLoading()}
                                fallback={
                                  <svg class="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" />
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                                  </svg>
                                }
                              >
                                <Show
                                  when={driveExpanded()}
                                  fallback={<HiOutlineChevronRight class="w-3 h-3" />}
                                >
                                  <HiOutlineChevronDown class="w-3 h-3" />
                                </Show>
                              </Show>
                            </span>

                            <Icon class="w-4 h-4 text-txt-muted shrink-0" />
                            <div class="flex-1 min-w-0">
                              <div class="text-xs text-txt truncate">
                                {drive.name || basename(drive.mountPoint)}
                                <span class="ml-1 text-2xs text-warning">(System)</span>
                              </div>
                              <div class="text-2xs text-txt-muted truncate">
                                {drive.mountPoint} · {drive.fileSystem.toUpperCase()}
                              </div>
                            </div>
                          </button>

                          {/* System drive directory tree */}
                          <Show when={driveExpanded()}>
                            <Show
                              when={driveChildren().length > 0}
                              fallback={
                                <Show when={!driveLoading()}>
                                  <div class="text-2xs text-txt-muted italic px-6 py-1">
                                    Empty or inaccessible
                                  </div>
                                </Show>
                              }
                            >
                              <For each={driveChildren()}>
                                {(entry) => <DirTreeNode entry={entry} depth={1} />}
                              </For>
                            </Show>
                          </Show>
                        </>
                      );
                    }}
                  </For>
                </div>
              </Show>
            </Show>
          </Show>
        </div>

        {/* ── Image Files Section ────────────────────────────── */}
        <div class="acquire-source-section">
          <button
            class="acquire-source-section-header"
            onClick={() => setImagesExpanded(v => !v)}
          >
            <span class="acquire-source-chevron" classList={{ "rotate-90": imagesExpanded() }}>▶</span>
            <HiOutlineDocument class="w-3.5 h-3.5 text-txt-muted" />
            <span class="flex-1 text-left">Image Files</span>
            <span class="text-2xs text-txt-muted">{props.imageFiles().length}</span>
          </button>

          <Show when={imagesExpanded()}>
            <Show
              when={props.imageFiles().length > 0}
              fallback={
                <div class="px-3 py-3 text-2xs text-txt-muted text-center">
                  <button
                    class="btn-text text-xs"
                    onClick={props.onAddImageFiles}
                  >
                    + Add image files
                  </button>
                </div>
              }
            >
              <div class="acquire-source-list">
                <For each={props.imageFiles()}>
                  {(path, index) => (
                    <div
                      class="acquire-source-item group"
                      classList={{ "acquire-tree-selected": queuedPaths().has(path) }}
                      onClick={() => toggleQueue(path, basename(path), false, 0)}
                      onContextMenu={(e) => handleImageContextMenu(path, e)}
                      title={`${path}\nClick to add/remove from queue · Right-click for options`}
                    >
                      <Show when={queuedPaths().has(path)}>
                        <HiOutlineCheckCircle class="w-3.5 h-3.5 text-accent shrink-0" />
                      </Show>
                      <HiOutlineDocument class="w-4 h-4 text-amber-400 shrink-0" />
                      <div class="flex-1 min-w-0">
                        <div class="text-xs text-txt truncate">{basename(path)}</div>
                        <div class="text-2xs text-txt-muted truncate">{path}</div>
                      </div>
                      <button
                        class="icon-btn-sm opacity-0 group-hover:opacity-100 transition-opacity"
                        onClick={(e) => {
                          e.stopPropagation();
                          props.onRemoveImageFile(index());
                        }}
                        title="Remove"
                      >
                        <HiOutlineXMark class="w-3 h-3" />
                      </button>
                    </div>
                  )}
                </For>
              </div>
            </Show>
          </Show>
        </div>

        {/* ── Acquisition Queue Section ───────────────────────── */}
        <div class="acquire-source-section">
          <button
            class="acquire-source-section-header"
            onClick={() => setQueueExpanded(v => !v)}
          >
            <span class="acquire-source-chevron" classList={{ "rotate-90": queueExpanded() }}>▶</span>
            <HiOutlineQueueList class="w-3.5 h-3.5 text-accent" />
            <span class="flex-1 text-left text-accent">Acquisition Queue</span>
            <Show when={queueCount() > 0}>
              <span class="acquire-queue-badge">{queueCount()}</span>
            </Show>
          </button>

          <Show when={queueExpanded()}>
            <Show
              when={queueItems().length > 0}
              fallback={
                <div class="px-3 py-4 text-2xs text-txt-muted text-center">
                  <p>No items queued</p>
                  <p class="mt-1">Browse drives above and click files or right-click → "Add to Queue"</p>
                </div>
              }
            >
              <div class="acquire-queue-list">
                <For each={queueItems()}>
                  {(item) => (
                    <div class="acquire-queue-item group">
                      <Show
                        when={item.isDir}
                        fallback={<HiOutlineDocument class="w-3.5 h-3.5 text-txt-secondary shrink-0" />}
                      >
                        <HiOutlineFolder class="w-3.5 h-3.5 text-amber-400 shrink-0" />
                      </Show>
                      <div class="flex-1 min-w-0">
                        <div class="text-xs text-txt truncate">{item.name}</div>
                        <div class="text-2xs text-txt-muted truncate">{item.path}</div>
                      </div>
                      <Show when={!item.isDir && item.size > 0}>
                        <span class="text-2xs text-txt-muted tabular-nums shrink-0">
                          {formatBytes(item.size)}
                        </span>
                      </Show>
                      <Show when={item.isDir}>
                        <span class="text-2xs text-txt-muted shrink-0">DIR</span>
                      </Show>
                      <button
                        class="icon-btn-sm opacity-0 group-hover:opacity-100 transition-opacity"
                        onClick={() => removeFromQueue(item.path)}
                        title="Remove from queue"
                      >
                        <HiOutlineXMark class="w-3 h-3" />
                      </button>
                    </div>
                  )}
                </For>
              </div>

              {/* Queue footer with actions */}
              <div class="acquire-queue-footer">
                <button
                  class="btn-sm text-2xs gap-1"
                  onClick={handleAcquireQueued}
                >
                  Acquire {queueCount()} item{queueCount() > 1 ? "s" : ""}
                </button>
                <button
                  class="btn-text text-2xs text-txt-muted"
                  onClick={clearQueue}
                >
                  <HiOutlineTrash class="w-3 h-3 inline" /> Clear
                </button>
              </div>
            </Show>
          </Show>
        </div>
      </div>

      {/* Context menu overlay */}
      <ContextMenu
        items={contextMenu.items()}
        position={contextMenu.position()}
        onClose={contextMenu.close}
      />
    </div>
  );
};

export default AcquireSourcePanel;
