...
describe("update release notes", () => {
  it("shows 0.1.112 and 0.1.113 when updating from an older version", () => {
    const notes = getBundledUpdateReleaseNotes("0.1.113", "0.1.111");
  ...
  it("shows only the next bundled release when current version is known", () => {
    const notes = getBundledUpdateReleaseNotes("v0.1.113", "0.1.112");
  ...
  it("keeps unknown future release bodies unchanged", () => {
    expect(
      mergeUpdateReleaseNotes("Remote release body", "0.1.114", "0.1.113")
    ).toBe("Remote release body");
  });

  it("uses bundled notes when the update manifest has no body", () => {
    const notes = mergeUpdateReleaseNotes("", "0.1.113", "0.1.111");
  ...
  it("does not duplicate matching remote notes", () => {
    const remoteNotes = [
      "## CORE-FFX 0.1.113",
      "- **Project open repair:** Existing note.",
    ].join("\n");

    expect(mergeUpdateReleaseNotes(remoteNotes, "0.1.113", "0.1.112")).toBe(
      remoteNotes
    );
  });
});
