...
const UFED_DIR: &str = "/Users/terryreynolds/1827-1001 Case With Data /1.Evidence/02606-0900_1E_00216P";
...
fn bench_container_detection() -> Vec<BenchResult> {
    ...
    for (name, path) in containers {
        ...
        println!("  ✅ {} - {} -> {:?}", name, format_duration(duration), detected);
...
fn bench_ad1_operations() -> Vec<BenchResult> {
    ...
    // Test: Get info (fast)
    {
        ...
        match ad1::info_fast(AD1_FILE) {
            Ok(info) => {
                let duration = start.elapsed();
                let details = format!("items={}, version={}", info.item_count, info.logical.image_version);
                println!("  ✅ Info Fast - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test: Get full info
    {
        ...
        match ad1::info(AD1_FILE, false) {
            Ok(info) => {
                ...
                println!("  ✅ Info Full - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test: Build tree
    {
        ...
        match ad1::get_tree(AD1_FILE) {
            Ok(tree) => {
                ...
                println!("  ✅ Get Tree - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test: Get root children (lazy loading first level)
    {
        ...
        match ad1::get_root_children_v2(AD1_FILE) {
            Ok(children) => {
                ...
                println!("  ✅ Get Root Children V2 - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test: Get children at address (lazy loading - subsequent levels)
    {
        ...
        match ad1::get_children_at_addr_v2(AD1_FILE, 0, "/") {
            Ok(children) => {
                let duration = start.elapsed();
                let details = format!("{} items", children.len());
                println!("  ✅ Get Children@Addr V2 - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test: List archive
    {
        ...
        match archive::list_zip_entries(ZIP_FILE) {
            Ok(entries) => {
                ...
                println!("  ✅ List ZIP - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test: Get entry count
    {
        ...
        match archive::get_zip_entry_count(ZIP_FILE) {
            Ok(count) => {
                ...
                println!("  ✅ Entry Count - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test: Get root entries (lazy)
    {
        ...
        match archive::get_zip_root_entries(ZIP_FILE) {
            Ok(entries) => {
                ...
                println!("  ✅ Root Entries - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
...
fn bench_document_viewers() -> Vec<BenchResult> {
    ...
    let test_files = vec![
        ...
        ("Rust", "/Users/terryreynolds/GitHub/CORE-1/src-tauri/src/lib.rs"),
        ("TOML", "/Users/terryreynolds/GitHub/CORE-1/src-tauri/Cargo.toml"),
    ];
    ...
    for (name, path) in &test_files {
        ...
        println!("    ✅ {} - {} -> {:?}", name, format_duration(duration), format);
        results.push(result.pass(duration, &details));
    }
    ...
    for (name, path) in &test_files {
        ...
        match info {
            Ok(info) => {
                ...
                println!("    ✅ {} - {} ({})", name, format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
                println!("    ❌ {} - FAIL: {}", name, e);
...
fn bench_spreadsheet_viewer() -> Vec<BenchResult> {
    ...
    let xlsx_files = find_files_with_extension("/Users/terryreynolds/1827-1001 Case With Data ", "xlsx", 1);
    let csv_files = find_files_with_extension("/Users/terryreynolds/1827-1001 Case With Data ", "csv", 1);
    ...
    if let Some(xlsx_path) = xlsx_files.first() {
        ...
        match spreadsheet::read_spreadsheet_info(xlsx_path) {
            Ok(info) => {
                ...
                println!("    ✅ Get Info - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
                ...
                if !info.sheets.is_empty() {
                    ...
                    match spreadsheet::read_sheet(xlsx_path, sheet_name, 0, 100) {
                        Ok(rows) => {
                            ...
                            println!("    ✅ Read Sheet - {} ({})", format_duration(duration), details);
                            results.push(result.pass(duration, &details));
                        }
                        Err(e) => {
    ...
    if let Some(csv_path) = csv_files.first() {
        ...
        match spreadsheet::read_csv(csv_path, Some(100)) {
            Ok((headers, rows)) => {
                ...
                println!("    ✅ Read CSV - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
...
fn bench_hex_viewer() -> Vec<BenchResult> {
    ...
    // Test reading chunk
    {
        ...
        match viewer::read_file_chunk(test_file, 0, Some(65536)) {
            Ok(data) => {
                ...
                println!("  ✅ Read 64KB - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    // Test header parsing
    {
        ...
        match viewer::parse_file_header(test_file) {
            Ok(metadata) => {
                let duration = start.elapsed();
                let details = format!("format={}, {} regions", metadata.format, metadata.regions.len());
                println!("  ✅ Parse Header - {} ({})", format_duration(duration), details);
                results.push(result.pass(duration, &details));
            }
            Err(e) => {
    ...
    if let Ok(entries) = std::fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            if path.is_file() {
                ...
                let sub = find_files_with_extension(&path.to_string_lossy(), ext, max - found.len());
                found.extend(sub);
            }
        }
    }
...
fn print_summary(results: &[BenchResult]) {
    ...
    for r in sorted.iter().take(15) {
        ...
        println!("  {} {:38} {:>10}", status, r.name, format_duration(r.duration));
    }
