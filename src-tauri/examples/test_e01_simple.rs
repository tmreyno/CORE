...
fn main() {
    let path = std::env::args()
        .nth(1)
        .unwrap_or("/Users/terryreynolds/Downloads/2020JimmyWilson.E01".to_string());
    ...
    match ewf::info(&path) {
        Ok(info) => {
            ...
            if let Some(evidence) = &info.evidence_number {
                println!("Evidence Number: {}", evidence);
            }
            ...
            if let Some(acq_date) = &info.acquiry_date {
                println!("Acquisition Date: {}", acq_date);
            }
            if let Some(sys_date) = &info.system_date {
                println!("System Date: {}", sys_date);
            }
