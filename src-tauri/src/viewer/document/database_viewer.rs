...
fn sanitize_table_name(name: &str) -> DocumentResult<String> {
    ...
    if name.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '.' || c == '-') {
        ...
        Err(DocumentError::Parse(format!("Invalid table name: {}", name)))
    }
}
...
pub fn get_database_info(path: impl AsRef<Path>) -> DocumentResult<DatabaseInfo> {
    ...
    let views: Vec<String> = stmt.query_map([], |row| row.get(0))
...
pub fn get_table_schema(db_path: impl AsRef<Path>, table_name: &str) -> DocumentResult<TableSchema> {
    ...
    // Get indexes
    let mut idx_stmt = conn.prepare(&format!("PRAGMA index_list({})", safe_name))
        .map_err(|e| DocumentError::Parse(format!("Failed to list indexes: {}", e)))?;
    
    let indexes: Vec<String> = idx_stmt.query_map([], |row| row.get(1))
...
pub fn query_table_rows(
    ...
    // Get column names
    let stmt = conn.prepare(&format!("SELECT * FROM {} LIMIT 0", safe_name))
