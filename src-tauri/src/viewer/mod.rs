...
// Re-export parsers
pub use parsers::{
    parse_7z_header, parse_ad1_header, parse_ewf_header, parse_gzip_header,
    parse_qcow2_header, parse_rar_header, parse_raw_header, parse_vhdx_header,
    parse_vmdk_header, parse_zip_header,
};
...
fn parse_header_by_format(header: &[u8], extension: &str, file_size: u64) -> Result<ParsedMetadata, ContainerError> {
    ...
    Ok(ParsedMetadata {
        ...
        version: None,
        fields: vec![
            MetadataField::new("File Size", format_size(file_size), "General"),
        ],
        regions: vec![],
    })
}
