...
pub fn parse_l01_file_tree(path: &str) -> Result<L01FileTree, ContainerError> {
    ...
    if !is_l01 && !is_lx01 {
        return Err(ContainerError::InvalidFormat(
            "Not a valid L01/Lx01 file".to_string(),
        ));
    }
    ...
    while offset < file_size && section_count < max_sections {
        ...
        if section_type == "ltree" {
            ...
            let compressed_size = section_size
                .saturating_sub(76) // section header
                .saturating_sub(48); // ltree sub-header
            ...
            let decompressed = decompress_zlib(&compressed, uncompressed_size)?;

            debug!("Ltree decompressed: {} bytes", decompressed.len());
    ...
    while line_idx < lines.len() {
        ...
        match current_category.as_str() {
            ...
            "entry" => {
                ...
                // Build path
                entry.path = build_entry_path(&entries, &entry);
