...
type ParsedSectionsResult = (Vec<SegmentFile>, Option<VolumeSection>, Vec<ChunkLocation>, Vec<StoredImageHash>, HeaderInfo);
...
impl EwfHandle {
    ...
    pub fn open(path: &str) -> Result<Self, ContainerError> {
        ...
        let volume = volume_info.ok_or_else(|| ContainerError::ParseError(
            "No volume section found in EWF container. \
             The container may be corrupted or incomplete.".to_string()
        ))?;
        debug!(
    ...
    fn global_to_segment_offset(global_offset: u64, segment_sizes: &[u64]) -> Result<(usize, u64), ContainerError> {
        let mut cumulative = 0u64;
        ...
        Err(ContainerError::ParseError(format!("Global offset {} beyond all segments", global_offset)))
    }
        ...
        while lo < hi {
            ...
            if mid + 1 < segment_count && self.segment_cumulative_sizes[mid + 1] <= global_offset {
                ...
                if offset_in_segment >= self.segments[mid].file_size {
                    return Err(ContainerError::ParseError(format!("Offset {} beyond segment {} size {}", 
    ...
    pub fn read_chunks_batch(&mut self, start_chunk: usize, count: usize) -> Result<Vec<Vec<u8>>, ContainerError> {
    ...
    pub fn read_at(&mut self, offset: u64, length: usize) -> Result<Vec<u8>, ContainerError> {
        ...
        if offset >= media_size {
            return Err(ContainerError::ParseError(format!("Offset {} beyond media size {}", offset, media_size)));
        }
    ...
    pub fn read_sectors(&mut self, start_sector: u64, count: u64) -> Result<Vec<u8>, ContainerError> {
    ...
    fn read_chunk_internal(&mut self, chunk_index: usize, use_cache: bool) -> Result<Vec<u8>, ContainerError> {
        // Check cache first
        if use_cache {
        ...
        let location = match self.chunk_table.get(chunk_index) {
            ...
            None => {
                ...
                if chunk_index >= expected_chunks {
                    return Err(ContainerError::ParseError(format!("Chunk {} beyond expected count {}", chunk_index, expected_chunks)));
                }
    ...
    fn read_section_descriptor(file: &mut File, offset: u64) -> Result<SectionDescriptor, ContainerError> {
    ...
    fn read_volume_section(file_pool: &mut FileIoPool, file_index: usize, offset: u64) -> Result<VolumeSection, ContainerError> {
    ...
    fn read_table_section(file: &mut File, offset: u64, size: u64, _sectors_base: u64) -> Result<TableSection, ContainerError> {
        file.seek(SeekFrom::Start(offset))?;
