...
impl DocxGenerator {
    ...
    fn add_classification_header(&self, docx: Docx, report: &ForensicReport) -> Docx {
        ...
        let para = Paragraph::new()
            .add_run(
                Run::new().add_text(classification).bold().size(24), // 12pt
            )
            .align(AlignmentType::Center);

        docx.add_paragraph(para).add_paragraph(Paragraph::new()) // Empty line
    }
    ...
    fn add_title(&self, docx: Docx, report: &ForensicReport) -> Docx {
        ...
        let title = Paragraph::new()
            .add_run(
                Run::new().add_text(&report.metadata.title).bold().size(36), // 18pt
            )
            .align(AlignmentType::Center);
        ...
        let sub = Paragraph::new()
            .add_run(
                Run::new().add_text(&subtitle).size(20), // 10pt
            )
            .align(AlignmentType::Center);
    ...
    fn add_evidence_section(&self, docx: Docx, report: &ForensicReport) -> Docx {
        ...
        docx.add_table(table).add_paragraph(Paragraph::new()) // Spacer
    }
        ...
        if !ev.collected_items.is_empty() {
            docx = docx.add_paragraph(
                Paragraph::new().add_run(
                    Run::new()
                        .add_text(format!("Collected Items ({})", ev.collected_items.len()))
                        .bold()
                        .size(22),
                ),
            );
    ...
    fn add_section_header(&self, docx: Docx, title: &str) -> Docx {
        docx.add_paragraph(
            Paragraph::new().add_run(Run::new().add_text(title).bold().size(28)), // 14pt
        )
    }
...
mod tests {
    ...
    fn full_report() -> ForensicReport {
        ...
        report.findings.push(
            Finding::new("F001", "Test finding", "Description of finding")
    ...
    fn test_docx_generate_full_report() {
        ...
        assert!(
            metadata.len() > 100,
            "Full report should be larger than 100 bytes"
        );
    }
