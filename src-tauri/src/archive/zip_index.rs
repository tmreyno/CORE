...
impl ZipIndex {
    ...
    fn find_eocd(file: &mut File, file_size: u64) -> Result<(u32, u64, u64), ContainerError> {
        ...
        let is_zip64 = eocd_pos >= 20 && buf[eocd_pos - 20..eocd_pos - 16] == [0x50, 0x4B, 0x06, 0x07];
        
        if is_zip64 {
            // ZIP64: Get EOCD64 location from locator
            let eocd64_offset = u64::from_le_bytes([
                buf[eocd_pos - 12], buf[eocd_pos - 11], buf[eocd_pos - 10], buf[eocd_pos - 9],
                buf[eocd_pos - 8], buf[eocd_pos - 7], buf[eocd_pos - 6], buf[eocd_pos - 5],
            ]);
            ...
            if eocd64_buf[0..4] != [0x50, 0x4B, 0x06, 0x06] {
                return Err(ContainerError::InvalidFormat("Invalid ZIP64 EOCD signature".to_string()));
            }
            
            let entry_count = u64::from_le_bytes([
                eocd64_buf[32], eocd64_buf[33], eocd64_buf[34], eocd64_buf[35],
                eocd64_buf[36], eocd64_buf[37], eocd64_buf[38], eocd64_buf[39],
            ]) as u32;
            
            let cd_size = u64::from_le_bytes([
                eocd64_buf[40], eocd64_buf[41], eocd64_buf[42], eocd64_buf[43],
                eocd64_buf[44], eocd64_buf[45], eocd64_buf[46], eocd64_buf[47],
            ]);
            
            let cd_offset = u64::from_le_bytes([
                eocd64_buf[48], eocd64_buf[49], eocd64_buf[50], eocd64_buf[51],
                eocd64_buf[52], eocd64_buf[53], eocd64_buf[54], eocd64_buf[55],
            ]);
            ...
            let cd_size = u32::from_le_bytes([
                buf[eocd_pos + 12], buf[eocd_pos + 13], buf[eocd_pos + 14], buf[eocd_pos + 15],
            ]) as u64;
            let cd_offset = u32::from_le_bytes([
                buf[eocd_pos + 16], buf[eocd_pos + 17], buf[eocd_pos + 18], buf[eocd_pos + 19],
            ]) as u64;
    ...
    fn parse_central_directory(
        ...
        while pos + 46 <= cd_buf.len() && index < entry_count {
            ...
            let compressed_size = u32::from_le_bytes([
                cd_buf[pos + 20], cd_buf[pos + 21], cd_buf[pos + 22], cd_buf[pos + 23],
            ]) as u64;
            let uncompressed_size = u32::from_le_bytes([
                cd_buf[pos + 24], cd_buf[pos + 25], cd_buf[pos + 26], cd_buf[pos + 27],
            ]) as u64;
            ...
            let crc32 = u32::from_le_bytes([
                cd_buf[pos + 16], cd_buf[pos + 17], cd_buf[pos + 18], cd_buf[pos + 19],
            ]);
            ...
            if parent_path.is_empty() {
                ...
                Self::ensure_parent_dirs(&mut root_entries, &mut children, &mut seen_dirs, &parent_path);
                
                children.entry(parent_path.clone())
                    .or_default()
                    .push(entry);
            }
    ...
    pub fn load(cache_path: &Path) -> Result<Self, ContainerError> {
        ...
        if magic != INDEX_MAGIC {
            return Err(ContainerError::InvalidFormat("Invalid index magic".to_string()));
        }
