...
pub fn find_case_documents(
    ...
    if !path.exists() || !path.is_dir() {
        debug!("Case document search: path does not exist or is not a directory: {}", base_path);
        return Vec::new();
    }
    ...
    documents.sort_by(|a, b| {
        match (&a.document_type, &b.document_type) {
            (CaseDocumentType::ChainOfCustody, CaseDocumentType::ChainOfCustody) => a.filename.cmp(&b.filename),
...
fn find_documents_recursive(
    ...
    for entry in entries.flatten() {
        ...
        debug!("Found case document: {} (type: {:?})", filename, document_type);
...
fn detect_document_format(filename_lower: &str) -> Option<String> {
    if filename_lower.ends_with(".pdf") {
        ...
        Some("ODT".to_string())
    } else if filename_lower.ends_with(".png") || filename_lower.ends_with(".jpg") 
              || filename_lower.ends_with(".jpeg") || filename_lower.ends_with(".tif") 
...
mod tests {
    ...
    fn test_detect_document_type_coc() {
        assert_eq!(detect_document_type_by_filename("25-049_COC_25-06988-08.pdf"), CaseDocumentType::ChainOfCustody);
        assert_eq!(detect_document_type_by_filename("chain_of_custody.pdf"), CaseDocumentType::ChainOfCustody);
        assert_eq!(detect_document_type_by_filename("Chain-Of-Custody-Form.pdf"), CaseDocumentType::ChainOfCustody);
        assert_eq!(detect_document_type_by_filename("COC-123.pdf"), CaseDocumentType::ChainOfCustody);
        assert_eq!(detect_document_type_by_filename("evidence_custody_form.pdf"), CaseDocumentType::ChainOfCustody);
    }
    
    #[test]
    fn test_detect_document_type_intake() {
        assert_eq!(detect_document_type_by_filename("evidence_intake_form.pdf"), CaseDocumentType::EvidenceIntake);
        assert_eq!(detect_document_type_by_filename("submission_form.docx"), CaseDocumentType::EvidenceIntake);
    }
    
    #[test]
    fn test_detect_document_type_notes() {
        assert_eq!(detect_document_type_by_filename("case_notes.txt"), CaseDocumentType::CaseNotes);
        assert_eq!(detect_document_type_by_filename("examiner_notes.pdf"), CaseDocumentType::CaseNotes);
    }
    
    #[test]
    fn test_detect_document_format() {
        assert_eq!(detect_document_format("file.pdf"), Some("PDF".to_string()));
        assert_eq!(detect_document_format("file.docx"), Some("DOCX".to_string()));
    ...
    fn test_extract_case_number() {
        assert_eq!(extract_case_number("25-049_COC_25-06988-08.pdf"), Some("25-049".to_string()));
        assert_eq!(extract_case_number("2025-12345.pdf"), Some("2025-12345".to_string()));
        assert_eq!(extract_case_number("case_notes.pdf"), None);
    }
    
    #[test]
    fn test_extract_evidence_id() {
        assert_eq!(extract_evidence_id("EV001_form.pdf"), Some("E001".to_string()));
        assert_eq!(extract_evidence_id("ITEM-5_receipt.pdf"), Some("E5".to_string()));
    ...
    fn test_find_case_documents() {
        ...
        File::create(&intake_path).unwrap().write_all(b"test").unwrap();
        File::create(&notes_path).unwrap().write_all(b"test").unwrap();
        File::create(&other_path).unwrap().write_all(b"test").unwrap();
    ...
    fn test_content_based_coc_detection_text() {
        ...
        let coc_docs: Vec<_> = docs.iter()
            .filter(|d| d.document_type == CaseDocumentType::ChainOfCustody)
            .collect();
        
        assert_eq!(coc_docs.len(), 1, "Should find 1 COC form via content detection");
