...
use super::traits::{
    ...
    HashableContainer, MountableContainer, SegmentInfo, SegmentMetadata, SegmentedContainer,
...
impl EvidenceContainer for Ad1Parser {
    ...
    fn verify(&self, path: &Path, algorithm: &str) -> Result<VerifyResult, ContainerError> {
        ...
        let expected =
            info.companion_log
                .as_ref()
                .and_then(|log| match algorithm.to_lowercase().as_str() {
        ...
        let verified = expected
            .as_ref()
            .map(|e| e.to_lowercase() == hash.to_lowercase());
...
impl TreeContainer for Ad1Parser {
    fn list_entries(&self, path: &Path) -> Result<Vec<TreeEntryInfo>, ContainerError> {
        ...
        let results: Vec<TreeEntryInfo> = tree
            .iter()
            .map(|entry| {
...
impl EvidenceContainer for EwfParser {
    ...
    fn info(&self, path: &Path, _include_tree: bool) -> Result<ContainerMetadata, ContainerError> {
        ...
        let stored_hashes: Vec<StoredHashInfo> = info
            .stored_hashes
            .iter()
        ...
        let segments = if info.segment_count > 1 {
            let segment_paths = crate::ewf::get_segment_paths(&path_str).unwrap_or_default();

            let files: Vec<String> = segment_paths
                .iter()
                .map(|p| p.to_string_lossy().to_string())
                .collect();

            let sizes: Vec<u64> = segment_paths
                .iter()
                .filter_map(|p| std::fs::metadata(p).ok().map(|m| m.len()))
                .collect();
...
impl EvidenceContainer for ArchiveParser {
    ...
    fn extract(&self, path: &Path, output_dir: &Path) -> Result<(), ContainerError> {
        let path_str = path.to_string_lossy();
        let output_str = output_dir.to_string_lossy();
        crate::archive::extract_zip(&path_str, &output_str).map(|_| ())
    }
}
...
impl EvidenceContainer for UfedParser {
    ...
    fn verify(&self, path: &Path, algorithm: &str) -> Result<VerifyResult, ContainerError> {
        ...
        let expected = info.stored_hashes.as_ref().and_then(|hashes| {
            ...
            hashes
                .iter()
                .find(|h| {
                    h.filename == filename && h.algorithm.to_lowercase() == algorithm.to_lowercase()
                })
                .map(|h| h.hash.clone())
        });
...
#[deprecated(
    since = "0.2.0",
    note = "Use containers::operations functions directly instead"
)]
#[allow(deprecated)]
