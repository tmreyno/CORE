...
fn parse_companion_log(log_path: &Path) -> Result<CompanionLogInfo, ContainerError> {
    ...
    let is_forensic_md5 = content_lower.contains("forensic md5")
        ...
        || content
            .lines()
            .any(|l| l.trim().starts_with("* ") && l.contains("From:") && l.contains("To:"));
    let is_hash_only = log_path
        .extension()
    ...
    for line in content.lines() {
        ...
        if let Some((key, value)) = parse_key_value(line) {
            match key.to_lowercase().as_str() {
                ...
                "acquisition started" | "start time" | "started" => {
                    info.acquisition_started = Some(value)
                }
                "acquisition finished" | "end time" | "finished" | "completed" => {
                    info.acquisition_finished = Some(value)
                }
        ...
        && info.segment_hashes.is_empty()
    {
        return Err(ContainerError::ParseError(
            "No useful information found in log file".to_string(),
        ));
    }
...
fn parse_hash_line(line: &str, check_verified: bool) -> Option<StoredHash> {
    ...
    let algorithms = [
        "md5", "sha1", "sha256", "sha512", "sha-1", "sha-256", "sha-512",
    ];
...
mod tests {
    ...
    fn test_extract_numeric_value() {
        assert_eq!(
            extract_numeric_value("Size: 1024 bytes", "Size:"),
            Some(1024)
        );
        assert_eq!(
            extract_numeric_value("Count: 42, more text", "Count:"),
            Some(42)
        );
        assert_eq!(
            extract_numeric_value("Sectors: 2048", "Sectors:"),
            Some(2048)
        );
