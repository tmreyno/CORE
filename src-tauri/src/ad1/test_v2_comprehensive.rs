...
mod tests {
    ...
    fn test_v2_session() {
            ...
            .enumerate()
        {
            println!(
                "  [{}] {:.2} GB",
                i + 1,
                seg.size as f64 / 1024.0 / 1024.0 / 1024.0
            );
        }
        println!("✅ PASSED");
    }
    ...
    fn test_v2_header() {
        ...
        println!(
            "Zlib chunk: {} bytes",
            session.logical_header.zlib_chunk_size
        );
...
#[cfg(test)]
mod more_tests {
    use crate::ad1::operations_v2::{
        decompress_file_data, get_children_at_addr, get_root_children,
    };
    use crate::ad1::reader_v2::SessionV2;
    use std::time::Instant;
        ...
        while !to_visit.is_empty() && files_tested < 3 {
            ...
            if let Ok(item) = session.read_item_at(addr) {
                if item.item_type != 0x05
                    && item.decompressed_size > 0
                    && item.decompressed_size < 100000
                {
                    let start = Instant::now();
                ...
                if item.first_child_addr != 0 {
                    to_visit.push(item.first_child_addr);
                }
                if item.next_item_addr != 0 && to_visit.len() < 50 {
                    to_visit.push(item.next_item_addr);
                }
            }
        }
        println!("✅ PASSED");
    ...
    fn test_v2_cache() {
        ...
        while !to_visit.is_empty() {
            ...
            if let Ok(item) = session.read_item_at(addr) {
                if item.item_type != 0x05
                    && item.decompressed_size > 1000
                    && item.decompressed_size < 50000
                {
                    println!(
                        "Testing with: {} ({} bytes)",
                ...
                if item.next_item_addr != 0 && to_visit.len() < 30 {
                    to_visit.push(item.next_item_addr);
                }
            }
        }
                ...
                if item.first_metadata_addr != 0 {
                    if let Ok(meta) = session.read_metadata_chain(item.first_metadata_addr) {
                        ...
                        for m in meta.iter().take(3) {
                            println!(
                                "     cat={}, key={}, len={}",
                                m.category, m.key, m.data_length
                            );
                        }
                    }
                }
