...
pub use partition::{
    detect_partition_table, get_partition_filesystem_type, PartitionEntry, PartitionTable,
    PartitionTableType,
};
pub use traits::{
    BlockDevice, BlockReader, FilesystemDriver, FilesystemInfo, FilesystemType, SeekableBlockDevice,
};
...
pub fn detect_filesystem_type(
    ...
    offset: u64,
) -> Result<FilesystemType, VfsError> {
    ...
    if bytes_read < 512 {
        tracing::debug!(
            "detect_filesystem_type: only read {} bytes, need 512",
            bytes_read
        );
    ...
    tracing::debug!(
        "detect_filesystem_type: first 16 bytes: {:02x?}",
        &buf[0..16]
    );
    ...
    tracing::debug!(
        "detect_filesystem_type: APFS magic check: 0x{:08X} (want 0x4E585342)",
        apfs_magic
    );
    ...
    if device.read_at(offset + 1024, &mut ext_buf).is_ok() {
        ...
        if magic == 0xEF53 {
            ...
            let feature_compat =
                u32::from_le_bytes([ext_buf[92], ext_buf[93], ext_buf[94], ext_buf[95]]);
            let feature_incompat =
                u32::from_le_bytes([ext_buf[96], ext_buf[97], ext_buf[98], ext_buf[99]]);
