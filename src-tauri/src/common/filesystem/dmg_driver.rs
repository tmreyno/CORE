...
use super::traits::{BlockDevice, BlockReader, SeekableBlockDevice};
...
impl DmgDriver {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self, VfsError> {
        ...
        let partition_names: Vec<String> = plist
            .partitions()
            .iter()
            .enumerate()
            .map(|(i, p)| {
    ...
    pub fn partition_data(&self, index: usize) -> Result<Vec<u8>, VfsError> {
        // Check cache first
        {
            ...
            if let Some(Some(data)) = cache.get(index) {
                tracing::debug!(
                    "DMG partition {} - using cached data ({} bytes)",
                    index,
                    data.len()
                );
        ...
        let data = {
            ...
            reader.partition_data(index).map_err(|e| {
                VfsError::IoError(format!("Failed to decompress partition {}: {}", index, e))
            })?
        };

        tracing::debug!(
            "DMG partition {} - decompressed {} bytes in {:.2}s",
            index,
            data.len(),
            decompress_start.elapsed().as_secs_f32()
        );
