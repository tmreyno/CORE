...
pub fn compare_projects(project_a: &FFXProject, project_b: &FFXProject) -> ProjectComparison {
    ...
    let unique_to_b =
        bookmark_diff.only_in_b.len() + note_diff.only_in_b.len() + evidence_diff.only_in_b.len();
...
/// Compare bookmarks
fn compare_bookmarks(
    bookmarks_a: &[ProjectBookmark],
    bookmarks_b: &[ProjectBookmark],
) -> BookmarkDiff {
...
fn compare_evidence(project_a: &FFXProject, project_b: &FFXProject) -> EvidenceDiff {
    let files_a: HashSet<_> = project_a
        .open_directories
        .iter()
        .map(|d| d.path.clone())
        .collect();
    let files_b: HashSet<_> = project_b
        .open_directories
        .iter()
        .map(|d| d.path.clone())
        .collect();
...
fn compare_activity(project_a: &FFXProject, project_b: &FFXProject) -> ActivityDiff {
    let types_a: HashSet<_> = project_a
        .activity_log
        .iter()
        .map(|a| a.action.clone())
        .collect();
    let types_b: HashSet<_> = project_b
        .activity_log
        .iter()
        .map(|a| a.action.clone())
        .collect();
...
fn notes_equal(a: &ProjectNote, b: &ProjectNote) -> bool {
    a.title == b.title && a.content == b.content && a.priority == b.priority && a.tags == b.tags
}
...
pub fn merge_projects(
    ...
    for bookmark_b in &project_b.bookmarks {
        if let Some(pos) = merged
            .bookmarks
            .iter()
            .position(|b| b.name == bookmark_b.name)
        {
